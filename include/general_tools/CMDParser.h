...
public:
    ...
    void bindVar(std::string flag, T& destination, size_t numEntries,
        ...
        if (std::is_same<T, int>() || std::is_convertible<T, int*>()) {
            ...
            type = STRING;
                   ...
                   std::is_convertible<T, double*>()) {
            type = DOUBLE;
        } else if (std::is_same<T, bool>() || std::is_convertible<T, bool*>()) {
            type = BOOL;
        }
        m_params.push_back({type, (void*)&destination, numEntries, description,
    ...
    void bindVar(std::string flag, T& destination, std::string description,
        ...
        m_params.push_back({type, (void*)arrayLoc, 0, description, flag,
...
private:
    ...
    typedef enum { INT, DOUBLE, STRING, BOOL } datatype;
