...
void Timers::logNamedTimers() {
    if (out_file == "") {
        for (auto i = named_timers.begin(); i < named_timers.end(); i++) {
            std::cout << i->m_scopeName << ": " << *(i->m_executionTime) << "us"
                      << std::endl;
        }
    } else {
        std::ofstream out(out_file, std::ofstream::out | std::ofstream::app);
        time_t currentTime = std::chrono::system_clock::to_time_t(
            std::chrono::system_clock::now());
        out << "====================================" << std::endl;
        out << "Begin Timer Log: "
            << std::put_time(std::localtime(&currentTime), "%Y-%m-%d %X")
            << std::endl;
        out << "====================================" << std::endl;
        if (Timers::toScreen) {
            for (auto i = named_timers.begin(); i < named_timers.end(); i++) {
                std::cout << i->m_scopeName << ": " << *(i->m_executionTime)
                          << "us" << std::endl;
                out << i->m_scopeName << ": " << *(i->m_executionTime) << "us"
                    << std::endl;
            }
        } else {
            for (auto i = named_timers.begin(); i < named_timers.end(); i++) {
...
void Timers::setLogFile(std::string filename) { Timers::out_file = filename; }
...
Timer::~Timer() {
    if (m_out != nullptr)
        *m_out = std::chrono::duration_cast<std::chrono::microseconds>(
                     hresClock::now() - m_initialTime)
                     .count();
}

/// Sets a new internal start time for the timer
void Timer::start() { m_initialTime = hresClock::now(); }
...
hresClockTimePoint Timer::getCurrentTime() { return hresClock::now(); }
