...
const GLfloat quad_vertex_buffer_data[] = {-1.0f, 1.0f, -1.0f, -1.0f,
                                           1.0f,  1.0f, 1.0f,  -1.0f};
GLuint quad_vao;
...
GLuint buildRenderShadersProgram(const std::string& vert,
...
int main(int argc, char* argv[]) {
    ...
    // Window::DefaultWindowFlags() | SDL_WINDOW_UTILITY = SDL_WINDOW_OPENGL |
    // SDL_WINDOW_SHOWN | SDL_WINDOW_RESIZABLE | SDL_WINDOW_UTILITY
    // SDL_WINDOW_UTILITY maintains WM compositing features
    GUIWindow MainWindow(
        "This Is A Test", params.height, params.width,
        Window::DefaultWindowFlags() |
            SDL_WINDOW_UTILITY);  // initializes SDL2, OpenGL, and ImGUI
    ...
    graphicsParams graphics_params = {gradientProg, renderProg,
                                      &gradient_speed};
...
bool parseCMD(int argc, char* argv[], cmdParams& params) {
    std::string size;  // HxW
    CMDParser parser;
    parser.bindVar<std::string>("-size", size, 1,
                                "<Height>x<Width> of the window");
    ...
    if (size.size() != 0) {
        ...
        if (x_index >= size.size()) {
            std::cout
                << "Incorrect format, -size requires format <Height>x<Width>"
                << std::endl;
            parser.printHelp();
            return false;
        }
        ...
        try {
            ...
            std::cout << "Could not convert " << height << " to integer height"
                      << std::endl;
            parser.printHelp();
            return false;
        }
        try {
            ...
            std::cout << "Could not convert " << width << " to integer width"
                      << std::endl;
            parser.printHelp();
            return false;
        }
...
GLuint buildRenderShadersProgram(const std::string& vert,
    ...
    auto attachShader = [shaderProg, &shaderObjectFiles](
                            const std::string& shader,
                            GLenum shaderType) -> void {
        GLuint shaderObj = glCreateShader(shaderType);
        shaderObjectFiles.push_back(shaderObj);

        if (shaderObj == 0) {
            throw std::runtime_error(
                std::string("Error creating shader type ") +
        ...
        if (!success) {
            ...
            throw std::runtime_error(
                std::string("Error compiling shader, shader type: ") +
                std::to_string(shaderType));
        }
    ...
    if (success == 0) {
        ...
        throw std::runtime_error(
            std::string("Error linking rendering shader program"));
    }
    ...
    for (auto i = shaderObjectFiles.begin(); i != shaderObjectFiles.end();
         i++) {
        glDeleteShader(*i);
    }
...
GLuint buildComputeShader(const std::string& comp) {
    ...
    if (program == 0) {
        throw std::runtime_error(
            std::string("Error creating compute shader program"));
    }
    ...
    if (success == 0) {
        ...
        std::cerr << "Error linking shader program: " << errorLog << std::endl;
        throw std::runtime_error(
            std::string("Error linking compute shader program"));
    }
...
void renderGraphics(void* _params) {
    ...
    if (WindowSizeUpdated || tex_out == 0) {
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32F, WindowWidth - 300,
                     WindowHeight, 0, GL_RGBA, GL_FLOAT, NULL);
        glBindImageTexture(0, tex_out, 0, GL_FALSE, 0, GL_WRITE_ONLY,
                           GL_RGBA32F);
    }
